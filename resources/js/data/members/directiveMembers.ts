import type { DirectiveMember } from "@/interfaces/member.interface";

const directiveMembers: DirectiveMember[] = [
  {
    src: "/imgs/directive/cesar-acuna.png",
    centerText: "César Acuña Peralta",
    description:
      "Nació en Tacabamba, provincia de Chota, departamento de Cajamarca, Perú, el 26 de abril de 1966. Cursó sus estudios primarios y secundarios en su localidad natal. Entre 1991 y 1995 cursó estudios superiores de ingeniería civil en la Universidad Privada San Pedro de la ciudad de Chimbote. Entre el 2002 y el 2003 cursó la maestría en Docencia Universitaria en la Universidad César Vallejo de Trujillo y entre 2005 y 2006 el doctorado en Gestión Universitaria en la Universidad Nacional Pedro Ruiz Gallo. Además, ha participado en diferentes congresos, cursos, seminarios y talleres. Es ingeniero y político. Se desempeñó como gobernador regional de Lambayeque en dos periodos, 2011 – 2014 y 2015 - 2018. Como congresista de la República en el 2020-2021 y durante el periodo parlamentario fue presidente de la Comisión de Presupuesto y Cuenta General de la República. Fue gerente general de la Universidad Señor de Sipán - USS del 2004 al 2010.  Actualmente es el director general del campus UCV Chiclayo.",
    bottomText: "Presidente fundador",
    slug: "cesar-acuna-peralta",
    signature: "/imgs/directive/firmas/firmacesar.png",
    socialLinks: [
      { provider: "tiktok", href: "https://www.tiktok.com/@cesaracunap" },
      { provider: "instagram", href: "https://www.instagram.com/cesaracunap" },
      { provider: "facebook", href: "https://www.facebook.com/CesarAcunaCAP" },
      { provider: "x", href: "https://x.com/cesaracunap" },
      { provider: "linkedin", href: "https://www.linkedin.com/in/cesaracunap" },
      { provider: "youtube", href: "https://www.youtube.com/@CesarAcunaPe" },
      { provider: "spotify", href: "https://open.spotify.com/artist/7jmjI9WcaSTdwRfNe16i7p" },
      { provider: "whatsapp", href: "https://www.whatsapp.com/channel/0029VbCKFnj7dmei5bsj4047" },
      { provider: "website", href: "https://www.cesaracuna.pe/" },
    ],
  },
  {
    src: "/imgs/directive/luis-valdez-para-web-scaled.png",
    centerText: "Luis Valdez Farías",
    bottomText: "Secretario Ejecutivo Nacional",
    slug: "luis-valdez-farias",
    socialLinks: [
      { provider: "facebook", href: "https://www.facebook.com/LuisValdezFarias" },
      { provider: "x", href: "https://x.com/luisvaldezf" },
      { provider: "instagram", href: "https://www.instagram.com/luisvaldezfari/" },
      { provider: "tiktok", href: "https://www.tiktok.com/@luisalbertovaldezfarias" },
    ],
  },
  {
    src: "/imgs/directive/richard-acuna.png",
    centerText: "Richard Acuña Núñez",
    bottomText: "Dirección Ejecutiva Nacional",
    slug: "richard-acuna-nunez",
    socialLinks: [
      { provider: "instagram", href: "https://www.instagram.com/richardacuna_", },
      { provider: "facebook", href: "https://www.facebook.com/RichardAcunaN" },
      { provider: "x", href: "https://x.com/richardacunan" },
      { provider: "linkedin", href: "https://www.linkedin.com/in/richard-acu%C3%B1a-n%C3%BA%C3%B1ez-07b07553", },
    ],
  },
  {
    src: "/imgs/directive/humberto-acuna.png",
    centerText: "Humberto Acuña Peralta",
    bottomText: "Secretario Nacional de Organización Partidaria",
    slug: "humberto-acuna-peralta",
    socialLinks: [
      { provider: "instagram", href: "https://www.instagram.com/humbertoacunap", },
      { provider: "facebook", href: "https://www.facebook.com/HAP.RegionLambayeque", },
      { provider: "x", href: "https://x.com/humbertoacunap" },
      { provider: "linkedin", href: "https://www.linkedin.com/in/humberto-acu%C3%B1a-peralta-47249383", },
      { provider: "tiktok", href: "https://www.tiktok.com/@hap_peru" },
    ],
  },
  {
    src: "/imgs/directive/jorge-gonzales.png",
    centerText: "Jorge Gonzáles Oré",
    bottomText: "Secretario Nacional de Coordinación Territorial",
    slug: "jorge-gonzales-ore",
    socialLinks: [
      { provider: "instagram", href: "https://www.instagram.com/jorgegonzalesore", },
      { provider: "facebook", href: "https://www.facebook.com/jgonzales.ore" },
      { provider: "x", href: "https://x.com/jgonzalesore" },
      { provider: "linkedin", href: "https://www.linkedin.com/in/jorge-gonz%C3%A1les-or%C3%A9-0a79a138", },
      { provider: "tiktok", href: "https://www.tiktok.com/@jorgegonzalesoreoficial" },
    ],
  },
  {
    src: "/imgs/directive/marisol-espinoza.png",
    centerText: "Marisol Espinoza Cruz",
    bottomText: "Gerencia General y Secretaria Nacional de la Mujer",
    slug: "marisol-espinoza-cruz",
    socialLinks: [
      { provider: "instagram", href: "https://www.instagram.com/espinozacruzmarisol", },
      { provider: "facebook", href: "https://www.facebook.com/MarisolEspinozaC", },
      { provider: "tiktok", href: "https://www.tiktok.com/@marisolespinozacruz", },
      { provider: "whatsapp", href: "https://api.whatsapp.com/send/?phone=51912261944&text&type=phone_number&app_absent=0" },
    ],
  },
  {
    src: "/imgs/directive/yessenia-lozano.png",
    centerText: "Yessenia Lozano Millones",
    bottomText: "Secretaria Nacional de Juventudes",
    slug: "yessenia-lozano-millones",
    socialLinks: [
      { provider: "instagram", href: "https://www.instagram.com/yessenialozanos",},
      { provider: "facebook", href: "https://www.facebook.com/YesseniaLozanoS", },
      { provider: "x", href: "https://x.com/yessenialozanos" },
      { provider: "linkedin", href: "https://www.linkedin.com/in/lozanoy" },
      { provider: "whatsapp", href: "https://whatsapp.com/channel/0029VbByx9UEKyZCsONMpn1c" },
      { provider: "tiktok", href: "https://www.tiktok.com/@yessenialozanos" },
      { provider: "website", href: "https://www.yessenialozano.com/" },
    ],
  },
  {
    src: "/imgs/directive/alejandra-loyola.png",
    centerText: "Alejandra Loyola Vera",
    bottomText: "Dirección de EFOP y Escuela Nacional de Personeros",
    slug: "alejandra-loyola-vera",
    socialLinks: [
      { provider: "linkedin", href: "https://www.linkedin.com/in/alejandra-loyola-vera-05ba3355", },
    ],
  },
  {
    src: "/imgs/directive/fernando-velasco.png",
    centerText: "Fernando Velasco",
    bottomText: "Secretario Nacional de Personeros",
    slug: "fernando-velasco",
    socialLinks: [
      { provider: "tiktok", href: "https://www.tiktok.com/@velasco2023" },
      { provider: "instagram", href: "https://www.instagram.com/fernandovelasco2023/", },
      { provider: "facebook", href: "https://www.facebook.com/VelascoAPP/" },
    ],
  },
];

export default directiveMembers;
